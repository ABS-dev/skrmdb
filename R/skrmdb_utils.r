...
.checkmatrix <- function(A,
  ...
  if (A[, !all(y_inc == cummax(y_inc))] &&
      A[, !all(y_inc == cummin(y_inc))]) {
    if (warn.me) message("skrmdb :: y is not monotonic. ",
                         "ED50 may be unreliable.")
    warning.monotonic <- TRUE
  }
  if (2 * A[, min(y_inc)] >= N || 2 * A[, max(y_inc)] <= N) {
    if (warn.me) message("skrmdb :: Dilutions fail to bracket the midpoint. ",
                         "ED50 is unreliable.")
...
.checkdata <- function(formula,
  ...
  if (ncol(A) != 3) {
    stop("skrmdb :: formula must be of the form y + n ~ x ",
         "where y, n, and x are all different.",
         call. = FALSE)
  }
...
.checkall <- function(formula,
  ...
  if (is.null(A)) {
    warning("skrmdb :: Calling this function with the parameters ",
            "y, n, x is depreciated.",
            call. = FALSE)
