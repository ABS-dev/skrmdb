...
#' \describe{
#'
#' \item{\code{eval}}{Evaluation method. One of 'ReedMuench', 'SpearKarb', or
...
skrmdb <- setRefClass("skrmdb",
                      fields = list(ed = "numeric",
                                    eval = "character"),
                      methods = list(initialize = function(ed = numeric(),
                                                           eval = character()) {
                      ...
                      print = function() {
                        if (length(.self$ed) == 0) {
                          ...
                          cat(paste0("ED50 by method ", .self$eval, "\n"))
                          cat(.self$ed, "\n")
                        }
                      }
                      ))
...
#' @name getED
#' @title Accessor to retrieve the numeric median effective dose from a skrmdb
#'   or sk object
#' @description This is an accessor function for retrieving the numeric value of
#'   effective from a skrmdb or sk data object object generated by ReedMuench,
#'   SpearKarb or DragBehr
...
#' temp1 <- DragBehr(y = c(0, 3, 5, 8, 10, 10),
#'                   n = rep(10, 6),
#'                   x = 1:6)
#' getED(temp1)
...
#' @description The sk object holds values for the Spear Karb estimator for
#'   median estimated dose. It extends the skrmdb data object with value for
#'   variance.
#' @details
#'
#' \describe{
...
sk <- setRefClass("sk",
                  ...
                  methods = list(
                    initialize = function(ed = numeric(),
                                          eval = "SpearKarb",
                                          sk.var = numeric()) {
                      if (eval != "SpearKarb") {
                        stop("[sk: validation] the estimator",
                             "must be \"SpearKarb\"")
                      } else {
